    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                ...
                @endif
                    <div>
                        <form method="post" action="{{ route('applicant.update', ['id' => $applicant->id]) }}" enctype="multipart/form-data">
                            @csrf
                            <div>
                                <label for="name" class="block font-medium text-sm text-gray-700">
                                    氏名<span class="text-red-500 text-xs">（必須）</span>
                                </label>
                                <x-input id="name" class="block mt-1 w-1/2" type="text" name="name" value="{{ $applicant->name }}" required/>
                            </div>
                            <div class="mt-4">
                                <x-label for="address" value="住所" />
                                <x-input id="address" class="block mt-1 w-full" type="text" name="address" value="{{ $applicant->address }}" />
                            </div>
                            <div class="mt-4">
                                <x-label for="email" value="メールアドレス" />
                                <x-input id="email" class="block mt-1 w-1/2" type="email" name="email" value="{{ $applicant->email }}" />
                            </div>
                            <div class="grid grid-cols-2">
                                <div class="mt-4">
                                    ...
                                    <x-input id="mobile_phone" class="block mt-1 w-9/12" type="text" name="mobile_phone" placeholder="090XXXXXXXX" value="{{ $applicant->mobile_phone }}"/>
                                </div>
                                <div class="mt-4">
                                    <label for="name" class="block font-medium text-sm text-gray-700">
                                        電話番号(自宅)<span class="text-red-500 text-xs">※ハイフンなしで</span>
                                    </label>
                                    <x-input id="home_phone" class="block mt-1 w-9/12" type="text" name="home_phone" placeholder="03XXXXXXXX" value="{{ $applicant->home_phone }}" />
                                </div>
                            </div>
                            <div class="grid grid-cols-2">
                                <div class="mt-4">
                                    ...
                                    @foreach($departments as $department)
                                        @if ($department->id === $applicant->department_id)
                                ...
                                <div class="mt-4">
                                    ...
                                    @foreach($reclites as $reclite)
                                        @if($reclite->id === $applicant->reclite_id)
                                            <option value="{{ $reclite->id }}" selected>{{ $reclite->name }}</option>
                                        @else
                            ...
                            <div class="grid grid-cols-2">
                                <div class="mt-4">
                                    ...
                                    @foreach($handlers as $handler)
                                        @if ($handler->id === $applicant->reclite_id)
                                ...
                                <div class="mt-4">
                                    ...
                                    @foreach($status_list as $key => $value)
                                        <option value="{{ $key }}" <?php if($applicant->status === $key) {echo "selected";} ?>>{{ $value }}</option>
                                    @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="mt-4">
                                ...
                                <textarea id="memo" class="block w-full mt-1 form-select rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" 
                                        name="memo">{{ $applicant->memo }}</textarea>
                            </div>
                            <div class="grid grid-cols-7">
                                <div class="mt-4 col-end-8 col-span-4 sm:col-span-3 sm:col-end-8 text-right">
                                    <x-button class="bg-blue-400">
                                        更新
                                    </x-button>
                                    <a href="{{ route('applicant.info', ['id' => $applicant->id]) }}" class="bg-gray-600 inline-flex items-center px-4 py-2 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-800 active:bg-gray-900 focus:outline-none focus:border-gray-900 focus:ring ring-gray-300 disabled:opacity-25 transition ease-in-out duration-150">
                                        キャンセル
                                    </a>
                                </div>
                            </div>
